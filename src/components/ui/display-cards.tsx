...
function DeploymentCard({
  ...
  return (
    <a
      ...
      className={cn(
        "group relative flex select-none flex-col justify-between rounded-xl border border-mono-800 bg-mono-900/80 backdrop-blur-md transition-all duration-500",
        ...
        // Carousel variant: no skew, fixed width, no gradient overlay
        isCarousel && "h-36 w-[280px] flex-shrink-0 px-4 py-3",
        // Stacked variant: with skew, responsive sizing, gradient overlay
        !isCarousel && [
          "h-32 sm:h-40 w-[16rem] sm:w-[22rem] -skew-y-[8deg] px-4 sm:px-5 py-3 sm:py-4",
    ...
    >
      ...
      <div className="flex items-center gap-3 relative z-10">
        ...
        <div className="flex flex-col">
          ...
          <p className={cn(
            "text-xs text-mono-500 truncate",
            isCarousel ? "max-w-[180px]" : "max-w-[140px] sm:max-w-[200px]"
          )}>
            {url.replace('https://', '')}
          </p>
        </div>
      ...
      <div className="flex items-center justify-between relative z-10">
        <p className={cn(
          "text-xs text-mono-500 truncate",
          isCarousel ? "max-w-[160px]" : "max-w-[120px] sm:max-w-[180px]"
        )} title={lastCommit}>
...
export default function DisplayCards({ deployments }: DisplayCardsProps) {
  // For stacked view: reverse so most important (first) appears on top
  const reversedDeployments = [...deployments].reverse();
  
  // Stacked card positions for lg+ screens
  const cardStyles = [
  ...
  return (
    <>
      ...
      >
        <div className="flex gap-4 overflow-x-auto pb-4 px-1 snap-x snap-mandatory scrollbar-hide -mx-6 px-6">
        ...
        {/* Scroll hint */}
        <p className="text-center text-xs text-mono-600 mt-2">
          Swipe to see more →
        </p>
      </motion.div>
      ...
      <motion.div 
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.6, delay: 0.2 }}
        className="hidden lg:grid [grid-template-areas:'stack'] place-items-center"
      >

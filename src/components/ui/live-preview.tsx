...
>
            ...
            <div className="flex-1 mx-4 max-w-xl">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-mono-950 rounded-lg border border-mono-800">
                <Globe className="w-3.5 h-3.5 text-mono-600" />
                <span className="text-xs text-mono-400 truncate font-mono">
      ...
      <motion.div
        ...
        transition={{ duration: 0.3 }}
        className="mt-6 p-4 rounded-xl bg-mono-900/50 border border-mono-800"
      >
        <div className="flex items-start gap-3">
          <span className="inline-flex items-center justify-center rounded-lg bg-mono-800 p-2.5 ring-1 ring-mono-700">
            {activeProject.icon || <Globe className="size-5 text-mono-300" />}
          </span>
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-mono-100">{activeProject.title}</h3>
            {activeProject.description && (
              <p className="mt-1 text-sm text-mono-400 line-clamp-2">{activeProject.description}</p>
            )}
            {activeProject.tags && activeProject.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {activeProject.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2 py-0.5 text-xs font-medium text-mono-400 bg-mono-800 rounded-md border border-mono-700"
                  >
                    {tag}
                  </span>
                ))}

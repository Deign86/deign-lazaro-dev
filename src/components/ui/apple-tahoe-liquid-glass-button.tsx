...
const GlassButton = React.forwardRef<HTMLButtonElement, GlassButtonProps>(
  ({ className, children, size, contentClassName, glassColor, ...props }, ref) => {
    ...
    const enableDesktopEffects = React.useSyncExternalStore(
      subscribeToDesktopEffects,
      hasDesktopEffects,
      getServerDesktopEffects
    );
    ...
    // Track cursor position relative to button for light reflection
        ...
        */}
        ...
        <style>{`
          ...
          */
          ...
          @media (hover: hover) {
            .btn-liquid:not(:disabled):hover .btn-liquid-lens::after {
              opacity: 1;
            }
          }

          /* Lightweight fallback for touch and other coarse-pointer devices. */
          @media (hover: none), (pointer: coarse) {
            .btn-liquid-lens {
              background:
                linear-gradient(135deg, rgba(255, 255, 255, 0.14), transparent 55%),
                ${glassColor || "oklch(from var(--background) l c h / 38%)"};
        ...
        <button
          ...
          ref={mergedRef}
          onMouseMove={enableDesktopEffects ? handleMouseMove : undefined}
          {...props}
        >
